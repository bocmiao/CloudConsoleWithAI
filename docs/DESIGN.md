...
- **页面**：没登录时整页是登录页（第一次是「创建管理员账号」），通过 HTTP 从别的机器访问时提示密码会明文传输；会话失效时任何接口返回 `code: login`，页面回到登录页。侧边栏显示当前账号和「退出」；有些说明按运行方式改变措辞（比如密钥保存在哪里）。添加服务器时可以直接粘贴私钥内容（Web 版读不到你电脑上的密钥文件；桌面版两种都行），私钥存 secrets.json；

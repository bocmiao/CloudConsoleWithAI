...
- 界面：侧边栏新增「网站统计」页，选站点和时间范围（24 小时 / 7 天 / 30 天），显示 5 个指标、请求数曲线（悬停看每个时间点，可以切换成表格）和 4 个排行（网址、国家、状态码、IP）；「让 AI 分析」把当前站点和时间范围带到 AI 助手里。
...
判断规则：已过期或不足 7 天为严重；会自动续签但不足 15 天（说明续签可能失败了）或者不会自动续签且不足 30 天为警告。结果缓存 5 分钟，执行清单或回滚后自动刷新。每一行都有对应的「让 AI 处理 / 开启 HTTPS / 开启自动续签」按钮，把问题带到 AI 助手里。

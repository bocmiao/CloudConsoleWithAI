...
没点「执行」之前，程序只**查看**服务器，不做任何修改；清单也保存在「建议」页。目前能自动执行的操作：添加 swap、清理旧日志、重启服务、重启容器，以及 1Panel 上的调整 PHP-FPM 进程数、调整 MySQL 内存参数、设置应用内存上限、固定 Java 应用的最大堆、备份应用或数据库；腾讯云上的操作见下面。详见 [设计文档第 18 节](docs/DESIGN.md)。

**腾讯云**：在「设置 → 腾讯云」填入一个子账号的 SecretId 和 SecretKey（页面上有创建步骤和要勾选的权限，不要用主账号）。之后在 AI 助手里可以：

- **上线网站**：「把 blog.example.com 接入 EO 并开 HTTPS，源站是我的服务器」→ 清单：添加 EdgeOne 加速域名 → DNSPod 解析切到 EdgeOne → 申请免费证书。EdgeOne 里还没有这个站点时，需要先在控制台添加站点并选好套餐（涉及计费）；

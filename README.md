...
**证书**：侧边栏「证书」页汇总 EdgeOne、腾讯云 SSL 证书和各台服务器 1Panel 里的 HTTPS 证书，并实际访问每个网站确认，快到期、已过期、没开自动续签或者续签失败的会标出来。申请证书交给会一直在线的一方自动续签：经过 EdgeOne 的网站用 EdgeOne 免费证书；1Panel 服务器让 1Panel 向 Let's Encrypt 申请（HTTP 验证，泛域名用 DNS 验证），开启自动续签并给网站开启 HTTPS。都能一键回滚。

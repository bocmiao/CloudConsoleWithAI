...
**网站访问统计**：「网站统计 → 访问日志」直接统计服务器上网站的访问日志（1Panel、宝塔或 Nginx）：每天的 PV、UV、独立 IP、请求数、爬虫、流量和错误，访客看了哪些页面、从哪里来、哪些 IP 访问最多、手机还是电脑。可以看全部网站合计，也可以只看一个网站；今天按小时看，也可以看 7 天、30 天。日志只在服务器上统计，只把汇总结果传回来。经过 EdgeOne 的网站会从 X-Forwarded-For 取真实访客 IP；「EdgeOne」页仍然可以看 EdgeOne 的请求数、流量、带宽和缓存命中率。也可以直接问 AI「我的网站这周访问量怎么样」。

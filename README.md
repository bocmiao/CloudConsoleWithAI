...
没点「执行」之前，程序只**查看**服务器，不做任何修改；清单也保存在「建议」页。目前能自动执行的操作：添加 swap、清理旧日志、重启服务、重启容器，以及 1Panel 上的调整 PHP-FPM 进程数、调整 MySQL 内存参数、设置应用内存上限、备份应用或数据库，详见 [设计文档第 18 节](docs/DESIGN.md)。
